...
source("config.r");
billing_header = c("UnixTimeStamp","ActiveSessions","VmDemand","VmReserved","VmActive","VmBilled");
sim_log= read.csv(SYSTEM_LOG_WITH_RI,sep=",",header=F);
...
tsdata.vmreserved = zoo(sim_log$VmReserved,sim_log$StampToPosxTime)
...
tsdata.merged = merge(tsdata.userrequest,tsdata.vmdemand,tsdata.vmreserved,tsdata.vmactive,tsdata.vmbilling)
tsdata.forecastedscaleup=zoo(forecastdata_scaleup$ForecastedSessions,sim_log$StampToPosxTime);
tsdata.capacity = merge(tsdata.userrequest,tsdata.forecastedscaleup,tsdata.vmdemand*120,tsdata.vmactive*120,tsdata.vmreserved * 120 ,tsdata.vmbilling*120)

pdf("simulation_ri_graphs.pdf")
...
tsdata.forgraph=(tsdata.vmreserved+tsdata.vmactive)*120;
lines(tsdata.forgraph, lwd=1.5, lty=2, col="green")
lines(tsdata.vmreserved*120, lwd=1.5, lty=2, col="black")
legend("topright", inset=.02, title="Legend", c("Request","Forecasted","ODI","Reserved"),lty=c(3,2,2,2), col=c("red","blue","green","black"),cex=0.75)
...
lines(tsdata.forgraph[i:j], lwd=1.5, lty=2, col="green")
lines(tsdata.vmreserved[i:j]*120, lwd=1.5, lty=2, col="black")
legend("topright", inset=.02, title="Legend", c("Request","Forecasted","ODI","Reserved"),lty=c(3,2,2,2), col=c("red","blue","green","Black"),cex=0.75)
i=i+180;
}
